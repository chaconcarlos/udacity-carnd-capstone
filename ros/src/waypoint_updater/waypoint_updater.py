...
MAX_ACCELERATION        =  0.8 # 8 m/s2 by a rate of 10 Hz
...
class WaypointUpdater(object):
  ...
  def on_current_pose_received(self, msg):
    ...
    """
    #rospy.logdebug("WaypointUpdater - New pose received. Pose: %s", msg)
  ...
  def get_closest_waypoint_index(self):
    ...
    for index, waypoint in enumerate(self.map_waypoints):
      distance = self.get_distance(current_position, waypoint.pose.pose.position)
  ...
  def publish(self):
    ...
    if (look_ahead_index > self.map_waypoints_count):
      look_ahead_index = next_waypoint_index + (self.map_waypoints_count - next_waypoint_index)
    ...
    for i in range(len(future_waypoints)):
      ...
      rospy.logdebug("WaypointUpdater - Velocity progression: %s", waypoint_velocity)
      self.set_waypoint_velocity(future_waypoints[i], self.target_velocity)
